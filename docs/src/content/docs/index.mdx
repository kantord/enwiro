---
title: enwiro
description: Make window management as useful and convenient as possible.
---

Enwiro aims to make window management as useful and convenient as possible
...
- [`enwiro-adapter-i3wm`](/adapters/available-adapters/i3wm/) supports i3
- [`enwiro-adapter-tmux`](/adapters/available-adapters/tmux/) supports tmux
...
For terminals there is a shortcut: set `enw shell` as your terminal emulator's
configured shell and every new terminal window opens inside the active
environment automatically, waiting while it is still being prepared. See
[Using enwiro as your terminal's shell](/launching-apps/#using-enwiro-as-your-terminals-shell).
...
enwiro by installing and configuring more cookbooks. See
[Cookbooks](/cookbooks/) for details.

List of currently available cookbooks:

- `enwiro-cookbook-chezmoi`: Use your chezmoi source directory as an environment
- [`enwiro-cookbook-git`](/cookbooks/available-cookbooks/git/): Generate environments using Git repositories
...
- `enwiro-bridge-activitywatch`: Report the active environment to [ActivityWatch](https://activitywatch.net/)

Bridges that declare the `listen` capability are started and supervised by the
daemon automatically; see [Bridges](/bridges/) for details.
